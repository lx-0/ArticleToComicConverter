      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {recentComics.map((comic) => (
          <Link key={comic.cacheId} href={`/comic/${comic.cacheId}`}>
            <a className="block p-3 bg-black/30 rounded-lg border border-purple-500/20 hover:border-purple-500/40 transition-colors">
              <h4 className="text-sm font-medium text-purple-200 truncate">{comic.title || "Untitled Comic"}</h4>
